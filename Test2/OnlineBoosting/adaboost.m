...
switch(mode)
    case 'train'
        ...
        for t=1:itt
            ...
            model(t).lambdaWrong = sum(estimateclass~=dataclass);
            model(t).lambdaCorrect = length(dataclass) - model(t).lambdaWrong;
            model(t).error = err;
            model(t).key = t;
            ...
            model(t).totalError=sum(estimateclasstotal~=dataclass)/length(dataclass);
          
            
            if(model(t).totalError ==0), break; end
        end
