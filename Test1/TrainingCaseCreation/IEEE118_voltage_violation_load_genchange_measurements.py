...
def dist_branch_trip(fromBus,toBus): #trip a line or two winding transformer KEYWORDS: trip line branch transformer tx
    psspy.branch_data(fromBus,toBus,r"""1""",[0,_i,_i,_i,_i,_i],[_f,_f,_f,_f,_f,_f,_f,_f,_f,_f,_f,_f,_f,_f,_f])
...
numberofoperatingpoint = 10000

#read the randomized operating points
GeneratedOC = np.genfromtxt('random_operating_conditions.csv', dtype=float, delimiter=',') 

# remove the old database for voltage measurment if it exists
databaseMeasurements = 'databaseMeasurements.csv'
if os.path.isfile(databaseMeasurements):
    os.remove(databaseMeasurements)
...
with silence(output):
    ...
    Vol_violation = []
    insecure_index = []
    with open(databaseMeasurements,'a') as f2:
        for i in range(0,len(GeneratedOC)):
            ...
            if N == 0:
                measurement.extend(bus_voltage) # save bus voltage
            else:
                measurement.extend(zerolistmaker(len(bus_voltage))) # if voltage collapses give voltage as zero
